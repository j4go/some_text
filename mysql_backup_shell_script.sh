...
if [ -z "$USER" ]; then
    echo 'Username not set in configuration.' 
    exit 1
fi

if [ -z "$HOST" ]; then
    echo "Host not set in configuration."  
    exit 2
fi
...
if [ -z "$MAX_BACKUPS" ]; then
    echo 'Max backups not configured.'  
    exit 4
fi
...
dbs=$(echo 'show databases' | mysql --host="$HOST" --user="$USER" -P 9272 --password=$PASS)
...
do
    ...
    mysqldump --force --opt --routines --triggers --max_allowed_packet=250M --user=$USER --password=$PASS --host=$HOST -P 9272 "$db" | $BKUP_BIN > "$backupDir/$backupFile"
    echo  
done

#chown lj:lj $BACKUP_DIR -R
